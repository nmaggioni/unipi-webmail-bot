...
async function headless(secrets) {
  ...
  // Outlook login
  await page.click('input[type=email][name=loginfmt]');
  await page.type(secrets.webmail_username + '@unipi.it');
  ...
  try {
    let dontrememberbtn = await page.waitForSelector('#idBtn_Back', {timeout: 1500});
    await page.click('#KmsiCheckboxField');
    await dontremember.click();
    await waitForPageLoad(page);
  } catch (e) {
  }
  /*try {
